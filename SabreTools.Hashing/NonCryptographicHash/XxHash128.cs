    ...
    public sealed unsafe class XxHash128 : NonCryptographicHashAlgorithm
    {
        ...
        public static bool TryHash(byte[] source, byte[] destination, out int bytesWritten, long seed = 0)
        {
            if (destination.Length >= sizeof(ulong) * 2)
            {
                ...
                {
                    Hash128 hash = HashToHash128(sourcePtr, source.Length, seed);
        ...
        private static Hash128 HashToHash128(byte* sourcePtr, int length, long seed = 0)
        {
        ...
        private Hash128 GetCurrentHashAsHash128()
        {
            ...
            else
            {
                ...
                {
                    current = HashToHash128(buffer, (int)_state.TotalLength, (long)_state.Seed);
                }
            }
        ...
        private static void WriteBigEndian128(in Hash128 hash, byte[] destination)
        {
            ...
            {
                low = HashOperations.ReverseBits(low, 64);
                high = HashOperations.ReverseBits(high, 64);
            }

            Array.Copy(BitConverter.GetBytes(low), 0, destination, 0, 8);
            Array.Copy(BitConverter.GetBytes(high), 0, destination, 8, 8);
        }
        ...
        private static Hash128 HashLength0To16(byte* source, uint length, ulong seed)
        {
            ...
            return new Hash128(XxHash64State.Avalanche(seed ^ BitFlipL), XxHash64State.Avalanche(seed ^ BitFlipH));
        }
        ...
        private static Hash128 HashLength1To3(byte* source, uint length, ulong seed)
        {
            ...
            uint combinedh = HashOperations.RotateLeft32((uint)HashOperations.ReverseBits(combinedl, 32), 13);
            ...
            return new Hash128(XxHash64State.Avalanche(keyedLo), XxHash64State.Avalanche(keyedHi));
        }
        ...
        private static Hash128 HashLength9To16(byte* source, uint length, ulong seed)
        {
            ...
            m128Low ^= HashOperations.ReverseBits(m128High, 64);
