...
namespace SabreTools.Hashing.NonCryptographicHash
{
    ...
    internal static unsafe class XxHashShared
    {
        ...
        public static byte[] DefaultSecret
        => [
        ...
        public static void Initialize(ref State state, ulong seed)
        {
            ...
            if (seed == 0)
            {
                DefaultSecret.CopyTo(state.Secret, SecretLengthBytes);
            }
            else
            {
        ...
        {
            hash ^= HashOperations.RotateLeft64(hash, 49) ^ HashOperations.RotateLeft64(hash, 24);
        ...
        public static void Append(ref State state, byte[] source, int offset, int length)
        {
            ...
            {
                Array.Copy(source, 0, state.Buffer, state.BufferedCount, source.Length);
            ...
            fixed (byte* sourcePtr = source)
            {
                ...
                if (state.BufferedCount != 0)
                {
                    ...
                    Array.Copy(source, 0, state.Buffer, state.BufferedCount, loadSize);
                ...
                if (source.Length - sourceIndex > NumStripesPerBlock * StripeLengthBytes)
                {
                    ...
                    Array.Copy(source, sourceIndex - StripeLengthBytes, state.Buffer, InternalBufferLengthBytes - StripeLengthBytes, StripeLengthBytes);
                }
                ...
                Array.Copy(source, sourceIndex, state.Buffer, 0, source.Length - sourceIndex);
        ...
        public static void DigestLong(ref State state, ulong* accumulators, byte* secret)
        {
            ...
            fixed (byte* buffer = state.Buffer)
            {
                ...
                else
                {
                    ...
                    Marshal.Copy(state.Buffer, InternalBufferLengthBytes - catchupSize, (IntPtr)(lastStripe + StripeLengthBytes), catchupSize);
                    Marshal.Copy(state.Buffer, (int)state.BufferedCount, (IntPtr)(lastStripe + catchupSize), (int)state.BufferedCount);
        ...
        public static void DeriveSecretFromSeed(byte[] destinationSecret, ulong seed)
        {
            ...
            {
                ulong defaultSecret = DefaultSecret[i + 0]
                    | ((ulong)DefaultSecret[i + 1] << 8)
                    | ((ulong)DefaultSecret[i + 2] << 16)
                    | ((ulong)DefaultSecret[i + 3] << 24)
                    | ((ulong)DefaultSecret[i + 4] << 32)
                    | ((ulong)DefaultSecret[i + 5] << 40)
                    | ((ulong)DefaultSecret[i + 6] << 48)
                    | ((ulong)DefaultSecret[i + 7] << 56);
                defaultSecret += seed;

                destinationSecret[i] = (byte)(defaultSecret & 0xFF);
                destinationSecret[i + 1] = (byte)((defaultSecret >> 8) & 0xFF);
                destinationSecret[i + 2] = (byte)((defaultSecret >> 16) & 0xFF);
                destinationSecret[i + 3] = (byte)((defaultSecret >> 24) & 0xFF);
                destinationSecret[i + 4] = (byte)((defaultSecret >> 32) & 0xFF);
                destinationSecret[i + 5] = (byte)((defaultSecret >> 40) & 0xFF);
                destinationSecret[i + 6] = (byte)((defaultSecret >> 48) & 0xFF);
                destinationSecret[i + 7] = (byte)((defaultSecret >> 56) & 0xFF);

                defaultSecret = DefaultSecret[i + 8]
                    | ((ulong)DefaultSecret[i + 9] << 8)
                    | ((ulong)DefaultSecret[i + 10] << 16)
                    | ((ulong)DefaultSecret[i + 11] << 24)
                    | ((ulong)DefaultSecret[i + 12] << 32)
                    | ((ulong)DefaultSecret[i + 13] << 40)
                    | ((ulong)DefaultSecret[i + 14] << 48)
                    | ((ulong)DefaultSecret[i + 15] << 56);
                defaultSecret -= seed;

                destinationSecret[i + 8] = (byte)(defaultSecret & 0xFF);
                destinationSecret[i + 9] = (byte)((defaultSecret >> 8) & 0xFF);
                destinationSecret[i + 10] = (byte)((defaultSecret >> 16) & 0xFF);
                destinationSecret[i + 11] = (byte)((defaultSecret >> 24) & 0xFF);
                destinationSecret[i + 12] = (byte)((defaultSecret >> 32) & 0xFF);
                destinationSecret[i + 13] = (byte)((defaultSecret >> 40) & 0xFF);
                destinationSecret[i + 14] = (byte)((defaultSecret >> 48) & 0xFF);
                destinationSecret[i + 15] = (byte)((defaultSecret >> 56) & 0xFF);
            }
        }
        ...
        public struct State
        {
            ...
            internal byte[] Secret = new byte[SecretLengthBytes];

            /// <summary>The internal buffer. Length is <see cref="InternalBufferLengthBytes"/>.</summary>
            internal byte[] Buffer = new byte[InternalBufferLengthBytes];
